...
squareColor = [1, 1, 0]; %color of square
square = [0, 5, 5, 0; 0, 0, 5, 5]; %base cooridinates of square
square = square - 2.5; %offset so we can rotate about the center

angle = 5; %rotate by this much per frame
pauseTime =  1/200; %frame time
rotX = [cosd(angle), -sind(angle)];
rotY = [sind(angle), cosd(angle)];
transX = 0; %how far from origin we are
transY = 0;
velX = rand + 0.5; %movement per frame
velY = rand + 0.5;
limitX = 250; %axis limits
limitY = 250;
...
theta = 35; %initial angle
vx0   = cosd(theta) * v0; %x component of velocity
vy0   = sind(theta) * v0; %y component of velocity

totalTime = -2 * vy0 / g; %total flight time
t = 0:0.01:totalTime; %time vector
vy = vy0 + g*t; %vector of y velocities along path

y = vy0 * t + 0.5 * g * t.^2; %y positions
x = vx0 * t; %x positions

y = y + 2.5; %account for origin
x = x + 2.5;

for ii = 1:length(t)
    transX = transX + vx0; %move object
    transY = transY + vy(ii);
    
    fill(square(1,:)+x(ii), square(2,:)+y(ii), squareColor) %draw square
    axis([-3, limitX+3, -3, limitY+3]); %draw axis
    
    square = [rotX * square; rotY * square];
    pause(pauseTime)
end
